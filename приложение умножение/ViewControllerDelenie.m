...
#import "ViewControllerDelenie.h"

@interface ViewControllerDelenie ()
...
@implementation ViewControllerDelenie

- (void)viewDidLoad {
    [super viewDidLoad];
    
    UIImageView* apple = [[UIImageView alloc] initWithFrame:CGRectMake(68, 240, CGRectGetWidth(self.view.frame) / 6, CGRectGetWidth(self.view.frame) / 6)];
...
- (void)randomNumbers {
    ...
    int i, j = 0, deliteli[100];
    for (i = 1; i < self.oneChislo; i++) {
        if (self.oneChislo % i == 0) {
            deliteli[j] = i;
            j++;
        }
    }
    self.twoChislo = deliteli[arc4random() % j];
